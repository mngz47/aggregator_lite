   ...
   var f = new FormData();
     ...
     f.append('comments','auto');  
	   ...
	   for(var a=0;a<fields.length;a++){
		     f.append(fields[a].getElementsByTagName('input')[0].value,
			      getName(field_names,index).value);
...
var aliexpress = ['ogTitle','formatedPrice','description','attrValue','skuPropertyImagePath','ogurl'];
   ...
   function fetch_Ali_product_2(url,text){
	...
	if(attr[aa].includes(aliexpress[a])){
	 ...
	 e('log').innerHTML += '<div><textarea id='+fields[a].getElementsByTagName('input')[0].value+'_'+ind+' >'+(attr[aa].split(':').length>2?attr[aa].split(':')[1]+':'+attr[aa].split(':')[2]:attr[aa].split(':')[1])+'</textarea> '+getFieldFetch(fields[a].getElementsByTagName('input')[0].value)+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
	...
	// e('log').innerHTML += '<div><textarea id='+attr[aa]+'_'+ind+' >'+attr[aa].split(':')[1]+'</textarea> '+getFieldFetch(attr[aa])+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
			...
			values[values.length] = (attr[aa].split(':').length>2?attr[aa].split(':')[1]+':'+attr[aa].split(':')[2]:attr[aa].split(':')[1]);
