...
function getName(names,ind){
 ...
 function fetch_field_2(text,index,elements,names,size){
   ...
   function fetch_product(url,text){
     ...
     // e('log').innerHTML += '<div style="display:none;" id=text_'+ind+' >'+text+'</div>EE Product Text<br>';
	  text = window.open(url).document.body.innerHTML;
	  e('log').innerHTML += '<textarea>'+text+'</textarea>Product Text Transform<br>';
