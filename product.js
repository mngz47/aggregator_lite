...
var aliexpress = ['ogTitle','formatedPrice','description','attrValue','skuPropertyImagePath','ogurl'];
   ...
   function fetch_Ali_product_2(url,text){
	...
	//alert(attr[aa]);
	 ...
	 e('log').innerHTML += '<div><textarea id="'+aliexpress[a]+'_'+ind+'" >'+(attr[aa].split(':').length>2?attr[aa].split(':')[1]+':'+attr[aa].split(':')[2]:attr[aa].split(':')[1])+'</textarea> '+getFieldFetch(aliexpress[a])+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
