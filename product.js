...
var aliexpress = ['ogurl','formatedPrice','attrValue','skuPropertyImagePath'];
   ...
   function fetch_Ali_product_2(url,text){
	...
	var data = scripts[b].substring(scripts[b].indexOf('data: {"actionModule":{'),scripts[b].indexOf('}}'));
	...
	if(attr[aa].includes(aliexpress[a])){
	 ...
	 e('log').innerHTML += '<div>('+aa+')<textarea>'+attr[aa]+'</textarea> '+fields[a].getElementsByTagName('input')[1].value+'<br><a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
	 e('log').innerHTML += '<div><textarea id='+attr[aa]+'_'+ind+' >'+vv+'</textarea> '+getFieldFetch(attr[aa])+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
				
			values[values.length] = attr[aa].split(':')[1];
		}
	}
	}		    
		  }  
	...
	if(values.length == fields.length && confirm(values.length+' fields')){
	  showProduct();
	  saveProduct(url);
	  clearProduct();
	  }else{
