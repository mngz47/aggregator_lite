   ...
   var f = new FormData();
     ...
     f.append('comments','auto');  
	   ...
	   for(var a=0;a<fields.length;a++){
		     f.append(fields[a].getElementsByTagName('input')[0].value,
			 getName([''+field_names[a]],index).value);
...
var aliexpress = ['ogTitle','formatedPrice','description','attrValue','skuPropertyImagePath','ogurl'];
   ...
   function fetch_Ali_product_2(url,text){
	...
	if(attr[aa].includes(aliexpress[a])){
	 ...
	 e('log').innerHTML += '<div><textarea id='+field_names[a]+'_'+ind+' >'+(attr[aa].split(':').length>2?attr[aa].split(':')[1]+':'+attr[aa].split(':')[2]:attr[aa].split(':')[1])+'</textarea> '+getFieldFetch(field_names[a])+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
