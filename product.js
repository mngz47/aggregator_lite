...
var aliexpress = ['ogurl','formatedPrice','attrValue','attrValue','skuPropertyImagePath'];
   ...
   function fetch_Ali_product_2(url,text){
	...
	if(attr[aa].includes(aliexpress[a])){
	 ...
	 e('log').innerHTML += '<div><textarea id='+fields[a].getElementsByTagName('input')[0].value+'_'+ind+' >'+attr[aa]+'</textarea> '+getFieldFetch(fields[a].getElementsByTagName('input')[0].value)+'<a href=# onclick="this.parentNode.className=\'block\';if(confirm(\'remove\')){this.parentNode.remove();}return false;" >xx</a></div>';	
